...
suite:
  ...
  notes: |
    ...
    Timestamp: 2025-03-27 12:37:06

    This test scenario adds two items to the cart, submits the order, and verifies the order placement using parameterized data for item IDs, quantities, and region.
     ...
     - POST /proxy/v1/orders (Submit the order using the region parameter from the data source.)
     - GET /proxy/v1/orders (Verify the order has been placed by checking the order list for the newly created order.)
  dataSources:
  - id: ds_1381380407_1744153830990_1158285811
    impl:
      $type: CSVDataSource
      encoding: null
      file: true
      fileObj:
        ...
        backupPath: CreateOrder_generated_data_source.csv
        path: "${project_loc:TestAssets}/CreateOrder_generated_data_source.csv"
      data: true
      columnNames:
      - Item ID 1
      - Quantity 1
  ...
  - $type: TestSuite
    ...
    tests:
    - $type: TestSuite
      name: "This test scenario adds two items to the cart, submits the order, and\
        \ verifies the order placement using parameterized data for item IDs, quantities,\
        \ and region."
      testID: 2
      tests:
        ...
        tool:
          ...
          serviceInfo:
            serviceDescriptor:
              ...
              location: http://54.70.107.94:4040/pda/api-docs/v1-proxy
          jsonBuilder:
            hasValue: true
            value:
          ...
          literal:
            use: 1
            text:
              MessagingClient_LiteralMessage: "{\"itemId\":12345,\"itemQty\":100}"
      ...
      - $type: RESTClientToolTest
        ...
        tool:
          ...
          literal:
            use: 1
            text:
              MessagingClient_LiteralMessage: "{\"itemId\":67890,\"itemQty\":250}"
              type: application/json
          mode: Form JSON
          literalQuery:
            isPropertiesRef: true
          ...
          literal:
            ...
            text:
              MessagingClient_LiteralMessage: "{\"region\":\"UNITED_STATES\",\"location\"\
                :\"New York\",\"receiverId\":\"RX987654321\",\"eventId\":\"EVT-2023-001\"\
                ,\"eventNumber\":\"EVTNUM-2023001\"}"
              type: application/json
          mode: Form JSON
          literalQuery:
            isPropertiesRef: true
