...
suite:
  ...
  notes: |
    ...
    Timestamp: 2025-04-24 16:11:26

    This test scenario adds two items to the cart using parameterized item IDs and quantities, submits the order with a specified region, and verifies the order placement by retrieving the list of orders.
     - POST /proxy/v1/cartItems (Add the first item to the cart using the item ID and quantity from the data source. Parameterize the item ID and quantity.)
     - POST /proxy/v1/cartItems (Add the second item to the cart using the item ID and quantity from the data source. Parameterize the item ID and quantity.)
     - POST /proxy/v1/orders (Submit the order with the items added to the cart. Use the region parameter set to 'LOCATION_1'.)
     - GET /proxy/v1/orders (Verify that the order has been placed by checking the list of orders.)
  dataSources:
  - id: ds_1745536304443_1631478245
    impl:
      $type: CSVDataSource
      file: true
      fileObj:
        path: PDA_AI_generated_generated_data_source_2.csv
      data: true
      columnNames:
      - Item ID 1
      - Quantity 1
  ...
  - $type: TestSuite
    ...
    tests:
    - $type: TestSuite
      name: "This test scenario adds two items to the cart using parameterized item\
        \ IDs and quantities, submits the order with a specified region, and verifies\
        \ the order placement by retrieving the list of orders."
      testID: 2
      tests:
      - $type: RESTClientToolTest
        ...
        tool:
          ...
          literal:
            use: 1
            text:
              MessagingClient_LiteralMessage: "{\"itemId\":123456,\"itemQty\":50}"
      ...
      - $type: RESTClientToolTest
        ...
        tool:
          ...
          literal:
            use: 1
            text:
              MessagingClient_LiteralMessage: "{\"itemId\":789012,\"itemQty\":150}"
              type: application/json
          mode: Form JSON
          literalQuery:
            isPropertiesRef: true
          ...
          literal:
            ...
            text:
              MessagingClient_LiteralMessage: "{\"region\":\"UNITED_STATES\",\"location\"\
                :\"New York\",\"receiverId\":\"RX123456\",\"eventId\":\"EV987654\"\
                ,\"eventNumber\":\"EN567890\"}"
              type: application/json
          mode: Form JSON
          literalQuery:
            isPropertiesRef: true
